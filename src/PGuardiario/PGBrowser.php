...
class PGBrowser
{
    ...
    public function visited($url)
    {
        ...
        if (array_search($url, $this->visited) !== false) {
            return true;
        }
        $this->visited[] = $url;
     ...
     * @throws \Exception
     */
    public function post($url, $body, $headers = ['Content-Type: application/x-www-form-urlencoded'])
    {
        if ($this->useCache && file_exists($this->cacheFilename($url . $body))) {
            if ($this->cacheExpired($url . $body)) {
            ...
            if ($headers) {
                $this->setHeaders(preg_replace('/(.*?:).*/', '\1', $headers));
            } // clear headers
        }
        $this->lastUrl = $url;
