...
class PGPage
{
    ...
    public function __construct($url, $response, $browser)
    {
        $this->url = $url;
        $this->html = $response;
        $this->parseResponse($response);
        $this->is_xml = (
            isset($this->headers['Content-Type']) && preg_match('/\bxml\b/i', $this->headers['Content-Type'])
        ) ? true : false;
    ...
    private function isXpath($q)
    {
        return preg_match('/^[\.#]?\//', $q);
    }

    private function setParser($body)
    {
